...
SERVER=$(hostname)
#Email configuration
TO_ADDRESS="your_email@domain.tld"
SUBJECT="Alert"
# Maximum amount of bandwidth (megabytes) that you want to consume in a given month before anti-overage commands are run
MAX='10'
# Interface that you would like to monitor ("eth0" is the public interface for Rackspace Cloud Servers)
INTERFACE="eth0"
...
function getusage {
        ...
        if [ $TOTUSAGE -ge $MAX ]; then
      	 ...
      	 BODY="The server $SERVER has reached the $TOTUSAGE mb of your $MAX mb limit of bandwidth!"	
		if [ "$TOTUSAGE" -gt "$MAX" ]; then
		#echo Your $Server has reached the $TOTUSAGE / $MAX limit of bandwidth!!!
...
function logevent {
        ...
        MESSAGE="$@"
        echo "$STRINGBASE $MESSAGE" >> bwlimit.txt
}
if [ $MAX == "" ]; then
